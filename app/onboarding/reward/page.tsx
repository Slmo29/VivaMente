...
export default function OnboardingRewardPage() {
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col px-6 pt-4 pb-6 max-w-lg mx-auto"
      style={{ background: "linear-gradient(180deg, #B2D8D8 0%, #F4F7F8 100%)" }}>
      <StepLines current={3} />

      {/* Medaglia + testo */}
      <div className={`flex flex-col items-center text-center -mt-4 transition-all duration-700 ${animato ? "opacity-100" : "opacity-0"}`}>
        <div style={{ animation: animato ? "pulse 2s ease-in-out infinite" : "none" }}>
          <svg width="130" height="128" viewBox="0 0 160 158" fill="none" xmlns="http://www.w3.org/2000/svg">
            <defs>
      ...
      >
        <div className="flex items-center gap-1.5">
          <LightBulb width={16} height={16} strokeWidth={1.5} color={COLORS.primary} />
      ...
      {/* Benefit grid */}
      <div className={`w-full mt-8 transition-all duration-700 delay-500 ${animato ? "opacity-100" : "opacity-0"}`}>
        <p className="text-sm font-bold text-ink mb-2">Con il tuo profilo ottieni:</p>
